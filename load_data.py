...
class MyDataset(Dataset):
    def __init__(self, im_list_x, im_list_y, loader=default_loader, exchange_bands=True, first_branch_channel=102):
    ...
    def __getitem__(self, index):
        ...
        str = self.imlist_x[index].replace("hazy", "clean")
        str = list(str)
        if str[-7] == "_":
            str[-7:-4] = ""
        else:
        ...
        im_label = torch.Tensor(im_label).permute(2, 0, 1)
...
im_train_list_x = glob.glob("./dataset/HD/train/hazy/*.tif")
im_train_list_y = glob.glob("./dataset/HD/train/clean/*.tif")
im_test_list_x = glob.glob("./dataset/HD/test/hazy/*.tif") #视情况切换为验证集 
im_test_list_y = glob.glob("./dataset/HD/test/clean/*.tif") #视情况切换为验证集 
