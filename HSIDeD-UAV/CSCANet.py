...
class Basic_Residual_Block(nn.Module):
    def __init__(self,BCB_channels=198,BCB_feature=198):
...
class Multiscale_Block (nn.Module):
    def __init__(self, input_channel=198):
        super(Multiscale_Block, self).__init__()
        self.input_channel = input_channel
        self.conv_sp_atten_1 = nn.Sequential( nn.Conv2d(in_channels=self.input_channel, out_channels=198, kernel_size=3, padding=1), nn.Sigmoid() )
        self.conv_sp_atten_2 = nn.Sequential( nn.Conv2d(in_channels=self.input_channel, out_channels=198, kernel_size=5, padding=2), nn.Sigmoid() )
        self.conv_atten = nn.Sequential( nn.Conv2d(in_channels=2 * self.input_channel, out_channels=198, kernel_size=3, padding=1), nn.Sigmoid() )
...
class CSCANet(nn.Module):
    def __init__(self, input_image_channel=197):
        super(CANet, self).__init__()
        self.input_image_channel = input_image_channel
        self.first_branch_channel = 198//3
        ...
        self.Basic_Residual_1 = Basic_Residual_Block(BCB_feature=198)
        self.Multiscale_Block_1 = Multiscale_Block()
        self.Basic_Residual_2 = Basic_Residual_Block(BCB_feature=198)
       
        
        self.conv_layer1 = nn.Sequential(nn.Conv2d(in_channels=132, out_channels=66, kernel_size=1), nn.ReLU() )
        self.conv_layer2 = nn.Sequential(nn.Conv2d(in_channels=132, out_channels=66, kernel_size=1), nn.ReLU() )
        self.conv_layer3 = nn.Sequential(nn.Conv2d(in_channels=66, out_channels=66, kernel_size=1), nn.ReLU() )
        self.conv_layer4 = nn.Sequential(nn.Conv2d(in_channels=132, out_channels=66, kernel_size=1), nn.ReLU() )
        self.conv_layer5 = nn.Sequential(nn.Conv2d(in_channels=132, out_channels=66, kernel_size=1), nn.ReLU() )
        self.conv_layer6 = nn.Sequential(nn.Conv2d(in_channels=66, out_channels=66, kernel_size=1), nn.ReLU() )
    ...
    def forward(self, Input_HSI):
	 ...
	 third_branch_output = self.conv_layer6(third_branch_output)
         ...
         output = merge_1[:, 0:197, :, :]
         return output
     
        
if __name__ == '__main__':

    mcplb = CSCANet(input_image_channel=197).cuda()
