...
class Basic_Residual_Block(nn.Module):
    def __init__(self,BCB_channels=468,BCB_feature=468):
...
class Multiscale_Block (nn.Module):
    def __init__(self, input_channel=468):
        super(Multiscale_Block, self).__init__()
        self.input_channel = input_channel
        self.conv_sp_atten_1 = nn.Sequential( nn.Conv2d(in_channels=self.input_channel, out_channels=468, kernel_size=3, padding=1), nn.Sigmoid() )
        self.conv_sp_atten_2 = nn.Sequential( nn.Conv2d(in_channels=self.input_channel, out_channels=468, kernel_size=5, padding=2), nn.Sigmoid() )
        self.conv_atten = nn.Sequential( nn.Conv2d(in_channels=2 * self.input_channel, out_channels=468, kernel_size=3, padding=1), nn.Sigmoid() )
...
class CSCANet(nn.Module):
    def __init__(self, input_image_channel=468):
        super(CANet, self).__init__()
        self.input_image_channel = input_image_channel
        self.first_branch_channel = 468//3
        ...
        self.Basic_Residual_1 = Basic_Residual_Block(BCB_feature=468)
        self.Multiscale_Block_1 = Multiscale_Block()
        self.Basic_Residual_2 = Basic_Residual_Block(BCB_feature=468)
       
        
        self.conv_layer1 = nn.Sequential(nn.Conv2d(in_channels=312, out_channels=156, kernel_size=1), nn.ReLU() )
        self.conv_layer2 = nn.Sequential(nn.Conv2d(in_channels=312, out_channels=156, kernel_size=1), nn.ReLU() )
        self.conv_layer3 = nn.Sequential(nn.Conv2d(in_channels=156, out_channels=156, kernel_size=1), nn.ReLU() )
        self.conv_layer4 = nn.Sequential(nn.Conv2d(in_channels=312, out_channels=156, kernel_size=1), nn.ReLU() )
        self.conv_layer5 = nn.Sequential(nn.Conv2d(in_channels=312, out_channels=156, kernel_size=1), nn.ReLU() )
        self.conv_layer6 = nn.Sequential(nn.Conv2d(in_channels=156, out_channels=156, kernel_size=1), nn.ReLU() )
    ...
    def forward(self, Input_HSI):
         ...
         third_branch_input = Input_HSI[:, 2*first_branch_channel:, :, :]    
	 ...
	 third_branch_output = self.conv_layer6(third_branch_output)
         ...
         output = self.Basic_Residual_2(merge_1)
...
if __name__ == '__main__':
    ...
    mcplb = CSCANet(input_image_channel=468).cuda()
